...
convertPattern onVar onWild p = runQ (p >>= convertM onVar onWild)
  where
    convertM onVar onWild pat = case pat of
      ConP n args -> Con (name n) <$> mapM (convertM onVar onWild) args
...
asExp (Con n ps) = AppE (AppE (c "Data.Pattern.Con") (LitE (StringL n))) (ListE (map asExp ps))
